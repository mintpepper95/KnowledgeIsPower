...
// To use, notice no braces if we want to pass string
<Congratulations name='Jason' age={28} ></Congratulations>

// In JSX, any non-string value (like numbers, booleans, objects, arrays, functions, etc.) **must be wrapped in braces `{}`**.
...
```js
// This is assumed to be a function body
(param) => {}

// We add round brackets to tell React we are actually returning an object
...
Component rendering - React generating a new description of what the UI should look like by returning the JSX, based on its current state/props. It's invoked when the state/props of a component changes. 
It happens entirely within the virtual DOM. React builds the new virtual DOM calling your component functions and creating virtual nodes. It then compares this virtual DOM with the previous virtual DOM. Then DOM reconciliation.

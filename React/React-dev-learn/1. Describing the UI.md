...
This means if the component key remains constant, React will not remount the component (unless their position within virtual DOM change) when doing DOM reconciliation. It will just identify the differences between the previous and current virtual DOM and then update parts of the real DOM that has changed.

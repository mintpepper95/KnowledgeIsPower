...
DOM reconciliation is how React updates the actual DOM by comparing the virtual DOM to the actual DOM.

#### How does key affect reconciliation and state?

Without key prop, React relies on position of component within virtual DOM during reconciliation to determine whether to unmount or remount the component.
...
This means if the component key remains constant, React will not remount the component (unless their position within virtual DOM change) when doing DOM reconciliation. It will just identify the differences and then update parts of the real DOM that has changed.

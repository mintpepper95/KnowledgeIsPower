...
The key helps React to reconcile (when a component re-renders, how React figures out to update the actual DOM) virtual DOM.
...
#### Virtual DOM
React has its virtual DOM, it applies changes on this virtual DOM whenever there are new changes (re-renders, triggered from prop/state change/key prop), then calculates the difference between virtual DOM and real DOM, and then modifies the real DOM, this is known as Reconciliation.

The key prop plays a role in Reconciliation.

React uses key prop to identify individual components. If key remains the same, React assume the element has not changed, and it will reuse the existing DOM node associated with that element, without unmounting and remounting.

With an unchanged key prop, if a component is moved to a new position within the virtual DOM, React will unmount and remount but also preserve its state.


Without key prop, React relies on position of components within virtual DOM during reconciliation to determine whether to unmount or remount.

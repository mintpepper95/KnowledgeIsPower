...
List items need `key` prop because it tells React which array item each component corresponds to. This is important when we need to manipulate the list items, e.g. update, delete... The key helps React to identify the list item throughout its life time and make the correct updates to DOM. 

...
[[#Why don't mirror props in state]]
#### Thinking about UI declaratively
...
In this case, React is re-rendering Chat when props change. However, `textarea` value comes from `useState`, which is initially empty. Since we didn't update text state via `setText`, therefore it remains the same.

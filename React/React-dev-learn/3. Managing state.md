...
export default function MovingDot() {
  ...
  return (
    <div
      ...
      style={{
        position: 'relative', /* placed relative to its normal position, we can now use top, bottom, left, right */
        width: '100vw', /* 100% of viewport width, entire screen horizontally */
        height: '100vh', /* 100% of the viewport height, entire screen vertically */
      }}>
      <div style={{
        position: 'absolute', /* relative position container becomes a reference point for absolutely positioned children, these children are now outside normal doc flow, means they float */
        backgroundColor: 'red',
        borderRadius: '50%',
...
Note absolute div item has a width of 20 and height of 20.
left and top being -10 means centering this item, since 10 is half of 20.

Imagine top and left of this element position, now we move its anchor point 10px to the left, then another 10px to the top. The center of this element will now be at the anchor point.
```
...
For each unique piece of state, you will have to choose the component that owns it. This is known as single source of truth. It means that for each piece of state, there is a specific component that holds that info. Here we lift up the shared state to the common shared parent, and pass it down.
...
export default function Scoreboard() { 
  ...
  return (
    <div>
      ...
      // Note we use arrow fn, don't just assign setPlayerIdx() as it will execute immediately and cause infinite loop re-render
      <button onClick={() => {
...
// Without key prop, when we reverse the Field elements, their state won't be reversed, meaning old Field 1 state tied to new Field 1, old Field 2 state tied to new Field 2, and so on...
```
...
#### Stale React state bug with setInterval
Recall JavaScript **closures "freeze" values** at the time they're created.
Stale states happen when we pass arrow functions that uses React states to `setTimeout`, `setInterval` and even listeners. 

Since React states don't update immediately, the state captured in `setTimeout` and `setInterval` might not be the latest.

```ts
const StaleStateComponent = () => {
  const [value, setValue] = useState("apple");
  const ref = useRef({});

  const handleStart = () => {
    // updated value is now "pear", but since state is a snapshot, still 'apple' in this render
    setValue("pear");
    ref.current = setInterval(() => {
      // It will always output 'apple', never 'pear'
      console.log(value);
    }, 100);
  };
...
  const handleStop = () => {
    // after clearing interval, it now logs 'pear'
...
Another example with custom hook

Hooks can return values, although they don't have to.

With hooks, React will re-render a component if
* Component prop changes
* A stateful value returned by the hook changes (eg. `useState` inside hook )
...
      // This introduces the stale state issue, elapsedTime is always old state due to closure
...
Closure remembers the stale `elapsedTime`, keeping adding 0.1 to it.
![[Pasted image 20250421151052.png]]
...
    const intervalId = setInterval(() => {
      // This fixes the stale state issue by passing in a function
      // This function will always be called with latest state value, so guaranteed elapsed time will always be the latest
      setElapsedTime((elapsedTime) => elapsedTime + 0.1);
    }, 100);
...
All components that are descendent of context provider will trigger re-render when context value changes, even if the actual value they receive is the same, because the reference is now different. (whether they are actually updated/remounted into the DOM will depend on virtual DOM comparison).

This is where `useMemo()` comes to the rescue. If property doesn't change, then `contextValue` will remain the same reference. Therefore stopping the re-render.
```ts
// if the context value changes to another object, also with same property 'John', it will trigger a re-render cause reference is different.
...
export function ThemeProvider({ children }) {
  ...
  // for performance, function that returns handlers to alter value.

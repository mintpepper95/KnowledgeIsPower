...
##### Why not mirror props in state
```ts
// In this case, if parent passes a different messageColoe
...
So basically if component sits at same place in the DOM tree, it will not be destroyed.
But if the component sits somewhere now, it will be unmounted and then mounted again, so state is destroyed.

The component does re-render, it just doesn't change the DOM.
So this is different to React.memo.

One is DOM nodes render, other is whether the component re-renders.
...
Because it's the same component at the same position, from React's view, it's the same counter. You can even return one of the two jsx based on some conditions. As long as the Counter sits at the same position in the UI tree, the state is retained. React doesn't know where you place conditions, all it sees is the tree you return.

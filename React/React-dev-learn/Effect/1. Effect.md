...
useEffect without deps run on every render.
useEffect with empty deps array runs only once when component is first mounted.
useEffect with deps runs when any of the deps change.
...
// Correct example, React will update the screen first, then run the effect. Notice this userEffect has no deps array, this means it will fire after every render. We can optimise it by adding `isPlaying` to the deps array such it only fires when 'isPlaying' changes.
const VideoPlayer = ({src, isPlaying}) => {
	const ref = useRef(null);
...
Ans : schedule, cancel, schedule, alarm

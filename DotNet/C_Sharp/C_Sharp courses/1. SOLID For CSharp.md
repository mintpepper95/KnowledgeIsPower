...
Write your code in the simplest way you know. Avoid applying SOLID as premature optimisation. If code turns out to be a pain to work with, then apply SOLID.
...
```cs
// Every new status code requires us to update this method
...
Extract out the hard coded value into a parameter
...
Make method as virtual so base class can override it
```cs
public class MessageService {
    public virtual string GetMessage() { return "Bad Request"; }
}

public class NotFoundMessageService : MessageService {
    public override string GetMessage() { return "Not Found"; }
}
```


Injection, extract out logic and dep inject it as another class. `MessageService` never changes, new class just implements `IMessageProvider`.
...
public class BadRequestProvider : IMessageProvider {
    public string GetMessage() { return "Bad Request"; }
}
...
public class MessageService {
    ...
    public string GetMessage() { return _provider.GetMessage(); }
}
```
...
You might have a question when looking at above example, `why not just implement a Rate method on the Policy class itself`?

If you put `Rate()` on the Policy class itself, you just turned Policy from a data container into a class that holds heavy business logic.

* Tight coupling to infra and deps, `Rate()` might need to call APIs, dbs etc... Now you have to inject these deps to Policy class. Makes your data model hard to serialise.
* It violates SRP. A policy class should only be concerned with policy definition - what a policy is. Calculating rates for policy is not part of policy definition.
* You might need Policy in other parts of code base which doesn't care about rating. 

![[Pasted image 20260519234451.png]]
Approach A - Deep and heavy inheritance where data and behaviours are fused together

![[Pasted image 20260519234536.png]]
Approach B - separate data models from business rules



By separating into RatingEngine, we can use reflection to eliminate switch.


This is the strategy factory pattern. We pulled `Rate()` into its own class `AutoPolicyRater`, `LandPolicyRater` etc... This is the strategy pattern, having a common interface for all the different ways of doing something. Used when you need to switch to different variants of something at run time, now we just call the `Rate()` method on the strategy interface. This fixes SRP.



We then created a factory for creating the raters via reflection. Without factory, we would have to manually instantiate raters based on policy type, this means RatingEngine will be tightly coupled to rater classes. This fixes Open closed.

One of the downside of using switch is:
* Class knows too much, class has to know every single concrete implementation
...
LSP essentially says: **"If you have to ask what it is before you can use it, you aren't treating it like its base type."**

Superclass should always be able to be swapped out for subclass without breaking things.

Car -> Ford Focus, you should always be able to use a Ford Focus where a Car is required, because it should have the same behaviours etc.

Car -> Bicycle, should not be allowed, because while a bicycle may have some of the behaviours, it functions differently to a Car.


The goal is to move from **Conditional Logic** (where the caller has to know everything) to **Delegated Logic** (where the object knows how to handle itself).

Liskov principle is the law, strategy pattern is the mechanism for obeying it.

When code uses `is`, `as` checks to change behaviour based on a concrete type, it's breaking LSP. You fix it by moving logic to different classes and introduce a common interface, this is often the strategy pattern.

`if (employee is Manager) { /* Do Manager Print */ }

Typically we can do one of the two things to fix LSP violation

Make `Print` a virtual method on `Employee` base class, override it in `Manager` and `Intern`, this is polymorphism.

Question - Maybe `Employee` class shouldn't be concerned with Printing?

To protect SRP, we extract the printing logic out of it. Then implement `IEmployeePrinter` interface and concrete strategies like `ManagerPrinter` and `InternPrinter`
...
```
This is a micro-violation of LSP. System behaves differently because of null type. We can replace it with a `NullPrinter`.


Note this is code smell that signals LSP, the violation happened in class design. 

A good mental test for LSP: _"If I replaced every instance of the base type with this subtype, would all existing tests still pass without knowing the difference?"_ If no — you've broken LSP.


**The real design question**

Ask whether printing is a _core behaviour_ of `Employee` or an _external concern_:

![[Pasted image 20260520002209.png]]

For something like printing/formatting, **Strategy is almost always the right call** — because presentation logic tends to vary for reasons completely unrelated to the business type hierarchy. You might want `HTMLManagerPrinter` and `PDFManagerPrinter` without touching `Employee` at all.

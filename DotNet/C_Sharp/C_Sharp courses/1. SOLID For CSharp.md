...
* It violates SRP. A policy class should only be concerned with policy definition. Calculating rates for policy is not part of policy definition.
...
This is the strategy factory pattern. We pulled `Rate()` into its own class `AutoPolicyRater`, `LandPolicyRater` etc... This is the strategy pattern, having a common interface for all the different ways of doing something. Used when you need to switch to different variants of something at run time, now we just call the `Rate()` method on the strategy interface.

We then created a factory for creating the raters via reflection. Without factory, we would have to manually instantiate raters based on policy type, this means RatingEngine will be tightly coupled to rater classes.
